...
export const KeepAlive = {
  ...
  setup(props, { slots }) {
    ...
    //缓存的是组件 -》 组件中有subtree -> subTree上有el元素 -》 移动到页面中
    ...
    return () => {
      ...
      if (cacheVNode) {
        ...
        vnode.shapeFlag = vnode.shapeFlag | ShapeFlags.COMPONENT_KEPT_ALIVE; //告诉他不要做初始化操作
        keys.delete(key);
        keys.add(key); //刷新缓存
      } else {
        keys.add(key);
      ...
      vnode.shapeFlag =
        vnode.shapeFlag | ShapeFlags.COMPONENT_SHOULD_KEEP_ALIVE; //这个组件不需要真的卸载，卸载的dom 临时存放到存储容器中存放
      return vnode;
    };
  },
};
